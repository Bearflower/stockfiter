...
> **Codex 适配（优先于本文其余历史内容）：** 仅在用户明确要求部署或输入 `/deploy` 时执行。使用本仓库真实的 `docker-compose.yml`、Dockerfile、服务名和健康检查；不要复用本文下方其他项目的路径、服务器、容器、密钥或命令。部署前必须确认环境、主机、远端目录、服务与回滚方案；构建失败立即停止。验证要覆盖产物摘要、容器镜像、版本标记、关键文件摘要和健康检查，容器运行不等于部署成功。生产服务器禁止运行回测。下文是历史参考，冲突内容不执行。
