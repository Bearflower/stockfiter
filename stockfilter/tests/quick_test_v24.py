...
def load_first_n_stocks(data_dir: str = 'data/backtest/baostocks_full', n: int = 300) -> List[Dict]:
    """加载前 N 只股票数据"""
    stock_data = []
...
def quick_test_v24(n_stocks: int = 300):
    """快速测试 V2.4"""
    print("="*80)
    print(f"V2.4 快速测试（前 {n_stocks} 只股票）")
    print("="*80)
    print("V2.4 参数设置（激进放宽）：")
    print("  - 缩量到放量时间窗口：60 天（从 25 天放宽）")
    print("  - 跌幅阈值：8%（从 12% 放宽）")
    print("  - 放量涨幅：3%（从 5% 放宽）")
    print("  - 量比最小值：1.2（从 1.5 放宽）")
    print("  - 量比最大值：15（从 12 放宽）")
    print("  - 缩量比率：80%（从 60% 放宽）")
    print("  - 流动性要求：2000 万（从 3000 万降低）")
    ...
    if patterns_exclude_2019:
        ...
        print(f"\n基于 {total_stocks} 只样本估算全量结果（3317 只股票）：")
        print(f"  估算总形态数：{estimated_total_patterns:.0f} 个")
        print(f"  估算排除 2019 后形态数：{estimated_exclude_2019:.0f} 个")
    ...
    # V2.2 全量结果已知：总 478 个，排除 2019 后 47 个
    v22_total = 478
    v22_exclude_2019 = 47
    
    if total_patterns > 0:
        # 估算 V2.4 全量结果
        v24_total_est = total_patterns * (3317 / total_stocks)
        v24_exclude_2019_est = len(patterns_exclude_2019) * (3317 / total_stocks)
        
        print(f"V2.2 全量结果：")
        print(f"  总形态数：{v22_total} 个")
        print(f"  排除 2019 后：{v22_exclude_2019} 个")
        
        print(f"\nV2.4 估算全量结果：")
        print(f"  总形态数：{v24_total_est:.0f} 个（增长 {(v24_total_est/v22_total-1)*100:.1f}%）")
        print(f"  排除 2019 后：{v24_exclude_2019_est:.0f} 个（增长 {(v24_exclude_2019_est/v22_exclude_2019-1)*100:.1f}%）")
    ...
    json_file = output_dir / f'quick_test_v24_{timestamp}.json'
